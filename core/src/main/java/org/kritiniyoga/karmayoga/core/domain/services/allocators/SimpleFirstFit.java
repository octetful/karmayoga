...
import org.kritiniyoga.karmayoga.core.domain.factories.ScheduleFactory;
...
public class SimpleFirstFit implements Allocator {
  ...
  private Tuple2<List<Schedule>, TreeSet<TimeSlot>> directlyAllocate(
      Tuple2<List<Schedule>, TreeSet<TimeSlot>> scheduleSlotTuple,
  ...
  private Tuple2<List<Schedule>, TreeSet<TimeSlot>> splitSlotAndAllocate(
      Tuple2<List<Schedule>, TreeSet<TimeSlot>> scheduleSlotTuple,
      TimeSlot currentSlot, Task task
  ) {
  ...
  private Tuple2<List<Schedule>, TreeSet<TimeSlot>> allocateTaskToSlot(
      Tuple2<List<Schedule>, TreeSet<TimeSlot>> scheduleSlotTuple,
      TimeSlot currentSlot, Task task
  ) {
  ...
  private Tuple2<List<Schedule>, TreeSet<TimeSlot>> applyFirstFitAlgorithm(
      Tuple2<List<Schedule>, TreeSet<TimeSlot>> scheduleSlotTuple,
      Task task
  ) {
  ...
  private Seq<Schedule> firstFitAllocation(
      Seq<Task> tasks, Seq<TimeSlot> slots,
      List<Schedule> schedules
  ) {
    return tasks.foldLeft(Tuple.of(schedules, TreeSet.ofAll(slots)),
        this::applyFirstFitAlgorithm)._1;
  }
